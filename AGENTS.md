...
- **AI pipeline is slow on CPU**: The full 4-model pipeline (`POST /api/full-analysis`) takes ~8 minutes on a Cloud VM with no GPU. The `gemma:7b` model may fail with an OOM error if system memory is under ~6.5 GiB. The pipeline degrades gracefully — other models still produce results.
- **Startup order**: Start PostgreSQL first, then Ollama (`ollama serve &`), then the backend. The backend will fail `ensureSchema()` silently if PostgreSQL is not running yet but will still start serving requests (database writes will fail).
- **Ollama models must be pre-pulled**: Run `ollama pull <model>` for each of the 4 models before using the AI pipeline. Models persist across sessions if the VM snapshot is saved.
